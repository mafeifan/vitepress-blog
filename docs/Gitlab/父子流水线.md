...
download_upstream_artifacts:
  ...
  # https://gitlab.prod.devops.dps.kone.cn/help/ci/pipelines/downstream_pipelines.md#fetch-artifacts-from-an-upstream-pipeline  
  needs:
  ...
  script:
    ...
    - git config --global user.email "gitlab-ci@kone.com"
    - git config --global --add safe.directory "*"
    - mkdir -p ~/.ssh && chmod 700 ~/.ssh
    - ssh-keyscan ${CI_SERVER_HOST} >> ~/.ssh/known_hosts && chmod 644 ~/.ssh/known_hosts
    - cat "$SSH_PRIVATE_KEY" >  ~/.ssh/kone_gitlab && chmod 500 ~/.ssh/kone_gitlab
